...
class BNN:
    ...
    def __init__(self, params):
        ...
        if params.get('sess', None) is None:
            ...
            self._sess = tf.compat.v1.Session(config=tf.compat.v1.ConfigProto(gpu_options=gpu_options))
        else:
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.sy_pred_mean2d = tf.compat.v1.reduce_mean(self.sy_pred_mean2d_fac, axis=0)
            self.sy_pred_var2d = tf.compat.v1.reduce_mean(self.sy_pred_var2d_fac, axis=0) + \
                tf.compat.v1.reduce_mean(tf.square(self.sy_pred_mean2d_fac - self.sy_pred_mean2d), axis=0)
            ...
            self.sy_layer = tf.compat.v1.placeholder(dtype=tf.float32,
                                               shape=[self.num_nets, None, self.layers[0].get_input_dim()],
                                               name="3D_training_inputs")
            self.sy_pred_layer = self.create_layer_tensors(self.sy_pred_in2d)
    ...
    def predict(self, inputs, factored=False, layer = False, *args, **kwargs):
        ...
        if len(inputs.shape) == 2:
            if layer:
                return self.sess.run(self.sy_pred_layer, feed_dict = {self.sy_pred_in2d:inputs})
            if factored:
    ...
    # def predict_last_layer(self, inputs):
    #     retrun self.sess.run
    ...
    def create_prediction_tensors(self, inputs, factored=False, *args, **kwargs):
        ...
        if inputs.shape.ndims == 2 and not factored:
            mean = tf.compat.v1.reduce_mean(factored_mean, axis=0)
            variance = tf.compat.v1.reduce_mean(tf.square(factored_mean - mean), axis=0) + \
                       tf.compat.v1.reduce_mean(factored_variance, axis=0)
            return mean, variance
        return factored_mean, factored_variance

    def create_layer_tensors(self, inputs):
        output = self._compile_last_layer(inputs)
        return output
    ...
    def _compile_last_layer(self, inputs):
        """Compiles the output of the network at the given inputs.
        ...
        for layer in self.layers:
            ...
            if i == num_layers-1:
                break
        # output = cur_out.eval(session = self.sess)
        return cur_out
    # very slow! should set up tensor to speed up the process
    ...
    def _compile_losses(self, inputs, targets, inc_var_loss=True):
        ...
        if inc_var_loss:
            ...
            total_losses = tf.compat.v1.reduce_mean(tf.compat.v1.reduce_mean(tf.square(mean - targets), axis=-1), axis=-1)
