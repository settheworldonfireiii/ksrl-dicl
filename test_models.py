...
if __name__ == "__main__":
    ...
    # python test_models.py --models /scratch.global/radke149/gpt2mla/dicl/rl-baselines3-zoo/rl-trained-agents/sac/HalfCheetah-v3_1/HalfCheetah-v3.zip --baseline src/dicl/data/D4RL_HalfCheetah_expert.csv
    ...
    policy_kwargs = dict(
    net_arch={
        "pi": [256, 256],       # match your actor.latent_pi layers
        "qf": [256, 256, 256],  # match critic.qf0/qf1 layers
       }
    )


  
    model = SAC.load(args.models[0])
    #state_dict = torch.load(args.models[0], map_location="cpu")
    model.policy.eval()
  
    print(evaluate_policy(
    model,            # your SAC model with loaded weights
    envs,              # the same env (Monitor or VecEnv)
    n_eval_episodes=10,
    deterministic=True
    ))
    ...
    mse = np.zeros((1, 17))
    for i in range(17):
        mse[0,i] = np.linalg.norm(obss[:,0,i] - Xs[:500,i])/500
    print("------------------------")
    print(obss[:30, :, :17])
    print("**************************")
    print(Xs[:30,:17])
    ...
    print("MSE ", mean_squared_error(obss[:,0,:17], Xs[:500,:17]))
    ...
    for model in args.models:
        ...
        #print(actor.keys())
        #torch.save(actor.state_dict(), "expert_actor_weights.pth")
    ...
    pre_actor = Actor(env)
    actor = ActorWrapper(pre_actor, 'cuda')
    actor.actor.load_state_dict(torch.load(args.models[0], weights_only=True))
