...
def main():
    ...
    while global_step <= args.total_timesteps:
        ...
        if global_step % args.interact_every == 0:
            print("GLOBAL STEP UPPER", global_step)
            for _ in range(args.interact_every):
                ...
                global_step += 1
                #print("GLOBAL STEP INCREASE ", global_step)
                pbar.update(1)
                ...
                for idx, trunc in enumerate(truncations):
                    ...
                    if trunc:
                        ...
                        #real_next_obs[idx] = infos['terminal_observation']
                        #print(real_next_obs[idx])
                    """
                #pdb.set_trace()
        ...
        if global_step > args.learning_starts:
            local_step = 0
            #pdb.set_trace()
            #print("GLOBAL AND LOCAL STEP BGN", global_step + local_step)
            #print("GLOBAL STEP BGM ", global_step)
            #print("LOCAL STEP BGN", local_step)
            #print("ARGS LEARNING STARTS ", args.learning_starts)
            for _ in range(args.interact_every):
                ...
                if (global_step + local_step)%250 == 0:                
                    for i in range(batches_to_train_on[0].observations.shape[0]):
                        #pdb.set_trace()
                        #print("GLOBAL AND LOCAL STEP ", global_step + local_step)
                        ...
                        #print("NEW Y ", y)
                        #print("NEW R ",  torch.tensor(tf.get_static_value(batches_to_train_on[0].rewards[i].cpu())).squeeze(0))
                        #print("ARGS LEARNING STARTS ", args.learning_starts)
                        #print("ARGS INTERACT EVERY ", args.interact_every)
                        #print("GLOBAL STEP ", global_step)
                        if startabatch == False:
                            xubatch = xu
                            ybatch = y
                            startabatch = True
                        else:
                            xubatch = np.vstack((xubatch, xu))
                            ybatch = np.vstack((ybatch, y))
                            #print("XUBATCH ",xubatch)
                            #print("UBATCH ", ybatch)
                            #bll.train(xubatch, ybatch)
                        #pdb.set_trace()
                ...
                if (global_step + local_step)%500 == 0:
                    ...
                    #print("LOCAL STEP ", local_step)
                    #my_dx.train(100)
                    print("XUBAtCH ", xubatch.shape[0])
                    print("YUUUUUBAtCH " , ybatch.shape[0])
                    bll = eqx.tree_at(lambda m: m.training, bll, True)
                    key = jax.random.PRNGKey(42)
                    bll.train(xubatch, ybatch, key=key)
                    bll = eqx.tree_at(lambda m: m.training, bll, False)
                    #pdb.set_trace()
                    #print("TRAINED LIKE IN AKHMAT!")
                    #post_var = my_dx.update_bays_reg()
                    #print (np.trace(post_var[0]))
                ...
                if (
                    ...
                    #print("STEPSTARTEDSAMPLING ", step_started_sampling)
