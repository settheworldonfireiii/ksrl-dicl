...
class BLL(eqx.Module):
    ...
    def __init__(self, model_type, obs_dim, act_dim,  dropout_rate=0.1, decay = 0.01, hidden_dim  = 200, optimizer = "adamw", horizon = 100,std = 1e-3, weights_variance = 1e2, max_logvar = 0.5, min_logvar = -10, training=True, key=None):
        if key is None:
    ...
    @eqx.filter_jit
    def loss_step(self, optimizer, opt_state, inputs, targets, key):
        """
        ...
        def loss_fn(model, subkey):
            ...
            # Gaussian NLL
            jax.debug.print("y = {}", preds[..., :dim])
            sqerr = jnp.mean(jnp.mean(((preds[..., :dim] - targets) ** 2)*jnp.exp(-logvar), axis = -1), axis = -1)
            varloss = jnp.mean(jnp.mean(logvar, axis=-1), axis=-1)
            jax.debug.print("mse = {}", sqerr)
            loss_value = jnp.mean(sqerr + varloss) 
            return loss_value + 0.01*jnp.sum(model.max_logvar - model.min_logvar)
        key, subkey = jax.random.split(key)
        #(loss_value, key), grads = eqx.filter_value_and_grad(loss_fn, has_aux = True)(self, subkey)
        loss_value, grads = eqx.filter_value_and_grad(loss_fn)(self, subkey)
        ...
        return new_model, opt_state, loss_value, key
    ...
    def train(self, inputs, targets, epochs=100, key=None):
        ...
        for i in range(epochs):
            #key, subkey = jax.random.split(key)
            self, opt_state, loss_value, key = self.loss_step(optimizer, opt_state, inputs, targets, key)
            """
